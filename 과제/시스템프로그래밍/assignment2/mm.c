 ...
 */
/* $begin mmmalloc */
//size에 따른 2Word단위 계산 후 해당 사이즈로 find_fit실행.
//만약 널값이 리턴될 경우 free space가 없는 상황이므로 sbrk실행 후 리턴되는 위치 사용
void *mm_malloc(size_t size)
{
    ...
    {
        size_m=size+OVERHEAD;
    }
    if(size_m%8!=0)
    {
        size_m=(size_m/DSIZE+1)*DSIZE;
    }
    //printf("size_m %d\n",size_m );
    ret=find_fit(size_m);
	if(ret==NULL)
    {
        //printf("null? " );
        //heap 추가 할당
        ret=extend_heap(MAX(CHUNKSIZE,size_m)/WSIZE);
        //printf("%p\n",ret );
    }
	place(ret,size_m);
	return ret;
...
// bp가 NULL이 아닐 경우 헤더와 푸터를 추출하여 free/alloc비트만 변경 후 coalesce진행
 ...
 */
 //오버플로우가 일어 나지 않도록 ptr의 사이즈와 Size인자를 비교하여 더 작은 값으로 메모리 카피하도록 함
void *mm_realloc(void *ptr, size_t size)
{
	int* temp=mm_malloc(size);
    if(temp==NULL)
        return NULL;
    if(ptr!=NULL)
    {
        //오버플로우 방지 코드
        if(GET_SIZE(HDRP(ptr))<size) size=GET_SIZE(HDRP(ptr));
